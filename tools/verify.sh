...
step "3/62  Save round-trip and respawn"
...
step "4/62  Combat lands its hits"
...
step "6/62  Ability gating"
run_scene_test "abilities locked, pickup grants" scenes/tests/Gating.tscn GATE \
  "starts locked; pickup grants; player and save agree" "--fixed-fps 60"

step "7/62  Enemies respect ledges"
run_scene_test "enemies stay on their platforms" scenes/tests/Ledge.tscn LEDGE \
  "no enemy fell into a generated gap"

step "8/62  Enemies can hurt the player"
run_scene_test "enemy lands a hit" scenes/tests/EnemyDamage.tscn ENEMYDMG \
  "combat is bidirectional, not one-sided"

step "9/62  Saved abilities survive a restart"
# Two processes on purpose: this failure only appears ACROSS a restart, which is
# the path a player takes and the one a single-process test cannot see.
rm -f "$APPDATA/Godot/app_userdata/LostCrownlike/savegame.tres" 2>/dev/null
timeout 120 "$GODOT" --headless --path "$GAME" scenes/tests/LoadedAbilities.tscn -- --seed >/dev/null 2>&1
OUT="$(timeout 120 "$GODOT" --headless --path "$GAME" scenes/tests/LoadedAbilities.tscn 2>&1)"
if grep -q "RESULT: PASS" <<<"$OUT"; then
  pass "an ability earned in a previous session is still there on boot"
else
  fail "saved abilities are lost on restart"
  grep -F "[LOADED]" <<<"$OUT" | tail -2
fi
rm -f "$APPDATA/Godot/app_userdata/LostCrownlike/savegame.tres" 2>/dev/null

step "10/62  Death with no checkpoint banked"
...
step "12/62  Pause actually pauses"
...
step "13/62  Progression survives a room change"
...
step "14/62  No node leak across many room changes"
...
step "15/62  Wall jump is real content"
...
step "16/62  Charge attack is a mechanic, not a flag"
...
step "17/62  Environmental hazards exist"
...
step "18/62  Save survives a long session"
...
step "19/62  State machine survives input mashing"
...
step "20/62  Wall slide, the state mashing cannot reach"
...
step "21/62  Combo window escalates and lapses"
...
step "22/62  Sounds are actually distinguishable"
...
step "23/62  Falling out of the level kills and recovers"
...
step "25/62  Dash invulnerability"
...
step "26/62  Camera follows the player"
...
step "27/62  Checkpoints do not re-announce"
...
step "28/62  Enemies that fall out are removed"
...
step "29/62  No redundant ability pickups"
# Also uncovered. A crystal on the floor that does nothing is the clearest
# signal that a game is not tracking what the player has done.
run_scene_test "owned abilities are not re-offered" scenes/tests/PickupSkip.tscn PICKUP \
  "a rebuilt room skips abilities the player already has, and still places the rest" "--fixed-fps 60"

step "30/62  Nothing leaves the movement plane"
...
step "31/62  The run has an ending"
...
step "33/62  Enemies can be told apart by behaviour"
...
step "34/62  The three enemy types across a whole run"
...
step "35/62  Enemies are a threat, and the opening is survivable"
...
step "36/62  The whole run can actually be played"
...
step "37/62  Wall jumps actually climb"
...
step "38/62  Individual chunks at full difficulty"
...
step "39/62  Continuing animations loop"
...
step "40/62  Finishing the run leads somewhere"
...
step "41/62  Every checkpoint stands on ground"
...
step "42/62  The model is still moving seconds later"
...
step "44/62  Parry, and perfect parry"
...
step "45/62  The dead are cleaned up"
...
step "46/62  The voice pool recycles"
...
step "47/62  A long run WITH A RENDERER stays quiet"
...
step "48/62  The dungeon is not silent"
...
step "49/62  The pause menu is in the game"
...
step "50/62  The game boots into a title"
...
step "51/62  The last room is a boss fight"
...
step "52/62  Blows leave a mark, and absorbed ones look different"
...
step "53/62  Keys can be rebound, and one key means one action"
...
step "54/62  Enemies face what they are attacking"
...
step "55/62  The floor has an underside, and no hole is dark"
...
step "56/62  An arena cannot be run past"
...
step "57/62  Enemies hold the ground they were placed on"
...
step "58/62  The game exports and starts as a binary"
...
step "59/62  Continue continues, and a new run is new"
...
step "60/62  There is music, and it moves"
...
step "61/62  Each act has its own atmosphere"
...
step "62/62  The engine stays quiet"
